 ...
 * - $meta          : array    (ex: ['description'=>'...','robots'=>'index,follow'])
 * - $pageStyles    : string   (balises <link> ou <style> spécifiques à la page)
 * - $pageScripts   : string   (balises <script> spécifiques, injectées en bas)
 * - $bodyClass     : string   (classes CSS optionnelles sur <body>)
 * - $content       : string   (HTML de la vue enfant déjà rendue via buffer)
 *
 * Session / Sécurité :
 * - Gère la session, un token CSRF basique, les flashes: $_SESSION['flash'][] = ['type'=>'success|danger|warning|info','text'=>'...']
 */
