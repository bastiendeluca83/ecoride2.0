<?php
/**
 * app/Views/home/_featured_rides.php
 * ----------------------------------
 * Widget affichant les 3 derniers trajets publiés.
 * 
 * Particularités :
 * - Connexion DB directe (ici en PDO) → utile pour un bloc statique sur la home.
 * - Pas de logique métier : juste un SELECT + affichage.
 * - On sécurise toutes les sorties avec htmlspecialchars().
 */

/* Connexion PDO via variables d'environnement (fallback valeurs locales) */
...
try {
    ...
    // Affiche un message clair si la DB n’est pas dispo
    echo '<div class="alert alert-danger">Erreur connexion DB : ' . htmlspecialchars($e->getMessage()) . '</div>';
    return;
}

/* Requête SQL :
   - On récupère les trajets (id, villes, date, prix, véhicule électrique, conducteur)
...
<section class="my-5">
  ...
  <?php else: ?>
    <!-- Cas vide -->
    <p class="text-muted mb-0">Aucun trajet pour le moment.</p>
  <?php endif; ?>
</section>
